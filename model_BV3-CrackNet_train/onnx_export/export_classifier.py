...
def load_classifier_model(weight_path, num_classes=5, in_channels=4, device='cpu'):
    """
    加载分类模型
...
def export_classifier_onnx(model, output_path, input_size=(224, 224), in_channels=4, opset_version=11):
    """
    导出分类模型为ONNX格式
    ...
    # 创建dummy输入
    dummy_input = torch.randn(1, in_channels, input_size[0], input_size[1])
...
def main():
    parser = argparse.ArgumentParser(description='导出分类模型为ONNX格式')
    parser.add_argument('--weight', type=str, 
                        default=None,
                        help='模型权重路径')
    parser.add_argument('--output', type=str, 
                        default='mobilenetv3_small_crack.onnx',
                        help='输出ONNX文件路径')
    parser.add_argument('--input-size', type=int, nargs=2, 
                        default=[224, 224],
                        help='输入尺寸 (H W)')
    parser.add_argument('--in-channels', type=int, default=4,
                        help='输入通道数 (3=仅RGB, 4=RGB+Mask)')
    ...
    if args.weight is None:
        args.weight = os.path.join(project_root, 'crack_classifier', 'res', 'crack_classifier', 'mobilenetv3_small', 'model_best.pth')
