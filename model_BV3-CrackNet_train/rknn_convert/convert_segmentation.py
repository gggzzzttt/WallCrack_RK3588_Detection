...
def convert_segmentation_to_rknn(
    onnx_path,
    output_path,
    calibration_data,
...
def accuracy_check(rknn, calibration_data, num_samples=5):
    ...
    if os.path.isdir(calibration_data):
        image_files = [os.path.join(calibration_data, f) 
...
def main():
    parser = argparse.ArgumentParser(description='分割模型ONNX转RKNN')
    parser.add_argument('--onnx', type=str, 
                        default='../onnx_export/bisenetv2_crack.onnx',
                        help='ONNX模型路径')
    parser.add_argument('--output', type=str, 
                        default='bisenetv2_crack.rknn',
                        help='输出RKNN路径')
    parser.add_argument('--calibration', type=str, 
                        default='../onnx_export/calibration_data/segmentation_calib_list.txt',
                        help='校准数据文件/目录')
    ...
    if not os.path.exists(args.onnx):
        ...
        print("   请先运行 onnx_export/export_segmentation.py 导出ONNX模型")
        return
    ...
    # 转换
    success = convert_segmentation_to_rknn(
        args.onnx,
        args.output,
        args.calibration,
